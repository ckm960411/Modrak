...
import { clearRoomReviews, setRoomReviews } from "store/slices/roomsSlice"
import searchFirestoreDoc from "utils/functions/searchFirestoreDoc"

const LoadMoreReviewsLength = 3 // 한번에 가져올 리뷰들의 개수

const useLoadingRoomReviews = ( accommodationId: string ) => {
  ...
  const reviews = useAppSelector(state => state.rooms.reviews)

  // 첫 리뷰들을 로드하는 함수
  const loadReviews = async () => {
    const { searchedData: reviewData } = await searchFirestoreDoc(`reviews/${accommodationId}`)
    ...
    for (let i = reviewData.reviews.length -1; i >= reviewData.reviews.length -3 && i >= 0; i--) {
      ...
      const review: RoomReviewType = reviewData.reviews[i]
      const { searchedData: userData } = await searchFirestoreDoc(`users/${review.userUid}`)
      const reviewWithUserInfo: RoomReviewWithUserInfo = {
        ...review,
        nickname: userData!.nickname,
        profileImg: userData!.profileImg,
      }
      dispatch(setRoomReviews(reviewWithUserInfo))
    }
  }
  ...
  useEffect(() => {
    if (loadMore === 1) {
      dispatch(clearRoomReviews()) // 처음 로드될 때 기존의 리뷰상태는 지움
...
export default useLoadingRoomReviews