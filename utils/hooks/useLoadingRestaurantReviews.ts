...
import { clearRestaurantReviews, setRestaurantReviews } from "store/slices/restaurantsSlice"
import searchFirestoreDoc from "utils/functions/searchFirestoreDoc"

const LoadMoreReviewsLength = 3 // 한번에 가져올 리뷰들의 개수

const useLoadingRestaurantReviews = ( restaurantId: string ) => {
  ...
  const loadReviews = async () => {
    ...
    for (let i = reviewData.reviews.length -1; i >= reviewData.reviews.length -3 && i >= 0; i--) {
      ...
      const review: RestaurantReviewType = reviewData.reviews[i]
      const { searchedData: userData } = await searchFirestoreDoc(`users/${review.userUid}`)
      const reviewWithUserInfo: RestaurantReviewWithUserInfo = {
        ...review,
        nickname: userData!.nickname,
        profileImg: userData!.profileImg,
      }
      dispatch(setRestaurantReviews(reviewWithUserInfo))
    }
  }
  ...
  useEffect(() => {
    if (loadMore === 1) {
      dispatch(clearRestaurantReviews()) // 처음 로드될 때 기존의 리뷰상태는 지움
...
export default useLoadingRestaurantReviews